	...
	if (!$exit) {
		...
		if (!$FastLoop -and ![string]::IsNullOrWhiteSpace($Config.ApiKey) -and
			...
			if (![string]::IsNullOrWhiteSpace($json)) {
				...
				$json = Get-Rest "https://api.mindminer.online" "type=setworker&apikey=$($Config.ApiKey)&worker=$($Config.WorkerName -replace ' ', '+')&data=$str"
