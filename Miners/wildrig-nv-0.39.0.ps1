		...
		if ($Algo) {
			...
			if ($Pool) {
				...
				[MinerInfo]@{
					...
					TypeInKey = $true
					API = "xmrig"
					URI = "https://github.com/andru-kun/wildrig-multi/releases/download/0.39.0/wildrig-multi-windows-0.39.0.7z"
					Path = "$Name\wildrig.exe"
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm) $hosts -R $($Config.CheckTimeout) --opencl-platform=$([Config]::nVidiaPlatformId) --no-adl --no-igcl --api-port=4068 $extrargs"
					Port = 4068
