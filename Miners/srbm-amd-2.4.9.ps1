		...
		if ($Algo) {
			...
			if ($Pool -and !($Pool.Name -match "mph" -and ("ethash", "etchash") -contains $_.Algorithm)) {
				...
				[MinerInfo]@{
					...
					ExtraArgs = $extrargs
					Arguments = "--algorithm $($_.Algorithm) --pool $pools --wallet $($Pool.User) --password $($Pool.Password) --tls $tls --api-enable --api-port $port --disable-cpu --disable-gpu-nvidia --disable-gpu-intel --disable-worker-watchdog --retry-time $($Config.CheckTimeout) $nicehash $extrargs"
					Port = $port
