...
$Cfg.Algorithms | ForEach-Object {
	if ($_.Enabled) {
		...
		if ($Algo) {
			...
			if ($Pool) {
				$fee = 3
				if ($_.Algorithm -match "cnv8" -or $_.Algorithm -match "Lyra2rev3") { $fee = 2.5}
				...
				[MinerInfo]@{
					...
					Type = [eMinerType]::AMD
					API = "teamred"
					URI = "https://github.com/todxx/teamredminer/releases/download/v0.3.10/teamredminer-v0.3.10-win.zip"
