...
$Cfg = [BaseConfig]::ReadOrCreate([IO.Path]::Combine($PSScriptRoot, $Name + [BaseConfig]::Filename), @{
	...
	ExtraArgs = $null
	Algorithms = @(
		# [AlgoInfoEx]@{ Enabled = $true; Algorithm = "aergo"; ExtraArgs="--opencl-launch 19x128" } not work on zpool
		...
		# [AlgoInfoEx]@{ Enabled = $true; Algorithm = "c11"; ExtraArgs="--opencl-launch 17x128" } not fully compatible with zpool
...
$Cfg.Algorithms | ForEach-Object {
	if ($_.Enabled) {
		...
		if ($Algo) {
			...
			$Pool = Get-Pool($Algo)
			if ($Pool) {
				if ($Pool.Name -notcontains "nicehash") {
					...
					[MinerInfo]@{
						...
						Type = [eMinerType]::AMD
						API = "xmrig"
						URI = "http://mindminer.online/miners/AMD/wildrig-multi-01211.7z"
