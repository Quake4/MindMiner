		...
		if ($Algo -and $AlgoDual) {
			...
			if ($Pool -and $PoolDual) {
				...
				[MinerInfo]@{
					...
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm) -p $($Pool.Hosts[0]):$($Pool.PortUnsecure) -w $($Pool.User) --pool_password $($Pool.Password) --a2 $($_.DualAlgorithm) --p2 $($PoolDual.Hosts[0]):$($PoolDual.PortUnsecure) --w2 $($PoolDual.User) --pool_password2 $($PoolDual.Password) --no_watchdog --nvidia 1 --amd 0 --intel 0 --nc 1 --update_frequency_ms 60000 --http_address 127.0.0.1 --http_port $port $extrargs"
					Port = $port
