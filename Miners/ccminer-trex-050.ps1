		...
		if ($Algo) {
			...
			if ($Pool) {
				...
				[MinerInfo]@{
					...
					API = "trex"
					URI = "http://mindminer.online/miners/nVidia/t-rex-050.zip"
					Path = "$Name\t-rex.exe"
					ExtraArgs = $extrargs
					Arguments = "-a $($_.Algorithm) -o stratum+tcp://$($Pool.Host):$($Pool.PortUnsecure) -u $($Pool.User) -p $($Pool.Password) --retry_pause $($Config.CheckTimeout) -b 127.0.0.1:4068 $extrargs"
					Port = 4068
