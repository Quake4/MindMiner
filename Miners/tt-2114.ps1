...
$Cfg.Algorithms | ForEach-Object {
	if ($_.Enabled) {
		...
		if ($Algo) {
			...
			$Pool = Get-Pool($Algo)
			if ($Pool -and ($Pool.Name -notcontains "nicehash" -or ($Pool.Name -contains "nicehash" -and $_.Algorithm -notmatch "lyra2v3"))) {
				...
				[MinerInfo]@{
					...
					API = "claymore"
					URI = "https://tradeproject.de/download/Miner/TT-Miner-2.1.14.zip"
