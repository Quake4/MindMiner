		...
		if ($Algo) {
			...
			if ($Pool) {
				...
				[MinerInfo]@{
					...
					Arguments = "-a $($_.Algorithm) -o stratum+tcp://$($Pool.Host):$($Pool.PortUnsecure) -u $($Pool.User) -p $pass -R $($Config.CheckTimeout) $N --no-simple-hr --donate 0 --api-bind=4068 $($_.ExtraArgs)"
					Port = 4068
